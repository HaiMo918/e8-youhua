...
var jyssxf	      = "field10386";	//交易所手续费收取万分之. jysxfsq1
var hysxf         = "field10384";	//会员手续费万分之.   zhjysxfsq
var jyyqf         = "field10387";	//交易延期费收取万分之 . jyyqfysq
...
var hysxfZH           = "field10388";		//综合会员手续费 zhhysxfZH
...
jQuery(document).ready(function(){
	...
	checkCustomize = function (){
		...
		if(workflowid==182||workflowid==184||workflowid==185||workflowid==186){
			if(exchange_v!='TPME'||company_v!='0'){
				alert("本流程仅支持交易所为TPME并且所属公司为融金汇银！");
				istj = false;
			}
		}
	...
	jQuery("#"+hysxf).change(function(){// 会员手续费优惠比例   (5,100]，最小变更单位5
		...
		if((val < 0 || val > 100)  ||  !computeObj.isPositiveNum(  val / 5) ){
			...
			alert("会员手续费优惠比例  (10,100]，最小变更单位10");
		}else{
			computeObj.change();
			var numStr = NoToChinese(val);
			jQuery("#"+hysxfZH).val(numStr);
	...
	jQuery("#"+jyyqf).change(function(){//交易延期费优惠比例  (25,100]，最小变更单位25
...
var computeObj = {
		...
		isSpecial3 : function(){   //判断是否总监特批    首次交易时间与当前日期的间隔,在三个月内 
			...
			var sxfVal1 	  =  jQuery("#"+jyssxf).val();  	//交易所手续费奖励比例(%)
			var zhjysx2		 =  jQuery("#"+hysxf).val();  	//会员手续费优惠比例(%)
			...
			if(yqfVal > 0 && yqfVal <= 100){
				...
				if((crjjzVal >= 0 && crjjzVal < 50) && (zhjysx2 > 20 && zhjysx2 <= 100)){
					specialVal = true;
					num = 3;
		...
		isSpecial : function(){ //判断是否总监特批    三个月以上
			...
			if(yqfVal > 0 && yqfVal <= 100){
				...
				if((crjjzVal >= 0 && crjjzVal < 50) && (zhjysx2 > 40 && zhjysx2 <= 100)){
					specialVal = true;
					num = 2;
