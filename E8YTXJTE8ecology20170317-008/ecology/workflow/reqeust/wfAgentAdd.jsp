...
<FORM name="frmmain" action="/workflow/request/wfAgentOperatorNew.jsp" method="post">
	...
	<wea:layout  >
		...
		<wea:item attributes="{'isTableList':'true'}">	
	        ...
	        <tr >
				<td height="44" class="fieldName"	width="160px"><!-- 开始日期, 时间  onshowAgentBDate-->
				<%=SystemEnv.getHtmlLabelName(740,user.getLanguage())%>, <%=SystemEnv.getHtmlLabelName(277,user.getLanguage())%>
				</td>
					<td class="field" colspan="3">
					<button type="button"   class="Calendar" id="SelectBeginDate" onclick="onshowPlanDate('beginDate','begindatespan')"></BUTTON> 
					<SPAN id="begindatespan"><%=beginDate%><%if (beginDate.equals("")) {%><img align="absmiddle" src="/images/BacoError_wev8.gif"><%}%></SPAN> 
					&nbsp;&nbsp;&nbsp;
					<button type="button"   class="Clock" id="SelectBeginTime" onclick="onshowMeetingTime(begintimespan,beginTime,'0')"></BUTTON>
					<SPAN id="begintimespan"><%if (beginTime.equals("")) {%><img align="absmiddle" src="/images/BacoError_wev8.gif"><%}else{%><%=beginTime%><%}%></SPAN> 
	        ...
	        <tr >
				...
				<td class="field" colspan="3">
					...
					<SPAN id="enddatespan"><%=endDate%><%if (endDate.equals("")) {%><img align="absmiddle" src="/images/BacoError_wev8.gif"><%}%></SPAN> 
					&nbsp;&nbsp;&nbsp;
					<button type="button" class="Clock" id="SelectEndTime" onclick="onshowMeetingTime(endtimespan,endTime,'0')"></BUTTON>
					<SPAN id="endtimespan"><%if (endTime.equals("")) {%><img align="absmiddle" src="/images/BacoError_wev8.gif"><%}else{%><%=endTime%><%}%></SPAN>  
...
function doSave(obj){
	...
	if(document.frmmain.beagenterId.value==0){
	   ...
	   Dialog.alert("<%=SystemEnv.getHtmlNoteName(83,user.getLanguage())%>");
		...
		return;
	}else if(!check_form(frmmain,"beginDate,beginTime,endDate,endTime")){		
		return;
	}else {
