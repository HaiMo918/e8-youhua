...
public static String getRandomCharData(int length){
	...
	for(int i=0;i<length;i++){
		int index=rand.nextInt(3);//n--这是结合于该随机数返回。必须为正数。  在方法调用返回介于0(含)和n(不含)伪随机，均匀分布的int值。
...
private static String checkLoginIdMsg(String id, String resourceid, boolean needJson,String type,int languageid){
	...
	private static String checkLoginId(String id){
		RecordSet RS = new RecordSet();
		String result = "";
		RS.executeSql("select id from HrmResource where loginid = '"+id+"'");
		if(RS.next()) result = RS.getString(1);
...
<%
	...
	try{
		...
		}else if(cmd.equalsIgnoreCase("checkLoginid")){
			
			String loginid = StringUtil.getURLDecode(request.getParameter("loginid"));

			result.append(checkLoginId(loginid));
			...
			if(sessionMap.get(loginid) != null){
				...
				boolean isChange = false;
	             ...
	             if(!validateRand.toLowerCase().equals(validatecode.trim().toLowerCase())){
					...
					if(StringUtil.isNotNull(id) && !id.equals("0")){
						//newPassword = manager.getRandomPassword();
						newPassword = getRandomCharData(6);						
...
//			result.append(list != null && list.size() > 0);
    		...
    		if(!"".equals(type)){
				...
				rs.executeSql("update hrmresource set passwordlock = '0' where id="+userid);
			}else{
