	...
	<HEAD>
		...
		<script language="javascript">
			...
			function checkLoginId(){
				...
				if(!loginid){
					Dialog.alert("<%=SystemEnv.getHtmlLabelNames("83594,18622", languageid)%>");
					return false;
				}else{				
					var isCunZai = common.ajax("cmd=checkLoginid&loginid="+loginid);
					if(isCunZai==''){
						Dialog.alert("<%=SystemEnv.getHtmlLabelNames("-10951,23084", languageid)%>");
						return false;
					}
				}
				...
				common.ajax("cmd=forgotPasswordCheckMsg&loginid="+loginid+"&randomstr="+randomString(6)+"&type="+type+"&languageid="+languageid, false, function(result){
					if(parseInt(result)){
						...
						//}
						Dialog.alert("<%=SystemEnv.getHtmlLabelName(125970, languageid)%>");
					}
			...
			function doNext(){	
				...
				if(validatecode =='<%=SystemEnv.getHtmlLabelName(84270, languageid)%>'){
					Dialog.alert("<%=SystemEnv.getHtmlLabelName(84270, languageid)%>");
					return;
				}
				...
				//if(isad==0){
					switch (type){
						case 0:
							//发送短信
							var ValicateCode = common.ajax("cmd=checkValicateCode&id="+id+"&loginid="+loginid+"&validatecode="+validatecode);
							if(ValicateCode!=''){
								...
								message = "<%=SystemEnv.getHtmlLabelName(128878, languageid)%>"+"!";
								Dialog.alert(message);
								break;
							}
				...
				<wea:group context="<%=SystemEnv.getHtmlLabelName(1361, languageid)%>" attributes="{'groupOperDisplay':'none'}">
					...
					<wea:item>
						<wea:required id="loginidspan" required="true">
							...
							<input type="text" class="InputStyle" name="loginid" style="width:50%" maxLength=30 onchange='checkinput("loginid","loginidspan")'  onkeydown="if(event.keyCode==13)return false;">
						</wea:required>
					</wea:item>
					...
					<wea:item>
						<wea:required id="validatecodespan" required="true">
							<input type="text" class="InputStyle" name="validatecode" style="width:50%" size="15"  value="<%=SystemEnv.getHtmlLabelName(84270, languageid)%>" onchange='checkinput("validatecode","validatecodespan")' onfocus="changeMsg(0)" onblur="changeMsg(1)" onkeydown="if(event.keyCode==13)return false;">
						</wea:required>
					...
					<wea:item>
						<select name="type" id="type" class="inputstyle" style="width:50%" onchange='checkinput("type","typespan")'>
							...
							<option value="2"><%=SystemEnv.getHtmlLabelName(125978, languageid)%></option><%if(showSendEmail){%><%}%>
						</select>
					</wea:item>
				</wea:group>
			</wea:layout>
