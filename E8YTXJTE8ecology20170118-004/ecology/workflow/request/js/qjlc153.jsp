...
if("1".equals(qjType)){
	...
	String djnjsql = "select sqr,SUM(qjsc) as djnj from formtable_main_31 a left join workflow_requestbase b on a.requestId = b.requestid where qjlxytx = '1' and a.qsrq >= '2017-01-01' and b.currentnodetype in (1,2) and sqr = '"+resourceid+"' group by sqr";
	rs.executeSql(djnjsql);
	if(rs.next()){
		djnj = Util.getDoubleValue(rs.getString("djnj"));
	}
	double xxnj = 0.00;//员工线下请假
	String xxqjsql = "select workcode,SUM(qjts) as xxnj from empxxqjd where qjlxytx = '1' and ksrq>='2017-01-01' and workcode = '"+workcode+"' group by workcode";
	...
	String djtxsql = "select sqr,SUM(qjsc) as djtx from formtable_main_31 a left join workflow_requestbase b on a.requestId = b.requestid where qjlxytx = '0' and a.qsrq >= '2017-01-01' and b.currentnodetype in (1,2) and sqr = '"+resourceid+"' group by sqr";
	...
	String xxtxsql = "select workcode,SUM(qjts) as xxtx from empxxqjd where qjlxytx = '0' and ksrq>='2017-01-01' and workcode = '"+workcode+"' group by workcode";
