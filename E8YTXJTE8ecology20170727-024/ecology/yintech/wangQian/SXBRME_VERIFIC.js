var	jyzh		  = "field14509";	//交易账号.
var subDateId	  = "field14507"; //协议提交日期 .	

var name 			  =  "field14510";		//客户姓名
var cardNumbe		  =  "field14511";		//身份证编号
var customerId		  =	 "field14512";		//客户ids
var exchange		  =  "field14513";		//所属交易所
var company			  =  "field14514";		//所属公司
var tradeDate		  =  "field14515";		//首次交易日期 .
var isNewCstm		  =  "field14516";		//是否新客户 .
var bank			  =	 "field14517";		//开户绑定银行开户行.
var bindBankCard 	  =  "field14518";		//开户绑定银行账号/卡号.
var remittBank 		  =	 "field14549";		//打款银行开户行.
var remittBankCard    =  "field14523";  	//打款银行账号/卡号.
var isBindBankAlike   =  "field14524";		//打款银行是否与绑定银行账号/卡号相同

var jyssxf	      = "field14525";	//交易所手续费收取万分之. jysxfsq1
var hysxf         = "field14526";	//会员手续费万分之.   zhjysxfsq
var jyyqf         = "field14527";	//交易延期费收取万分之 . jyyqfysq
var crjjz         = "field14530";	//累计出入金净值不低于 正整数，默认为空 .

var special		  = "field14534"; //是否特批 0-是,1-否 .
var exceed	      = "field14535"; //是否超限 0-是,1-否 .

var nameZhVal 		  = "field14550";		//姓名中文表单值
var nameZhSpan 	      = "field14550span";	//姓名中文显示
var beginDate 		  = "field14528";       //开始日期
var beginDateInput    = "field14551";		//开始日期中文表单
var beginDateSpan     = "field14551span";	//开始日期显示
var endDate 		  = "field14529"; 	    //结束日期
var endDateInput      = "field14552";	    //结束日期中文表单
var endDateSpan       = "field14552span";	//结束日期显示
var jyssxfZH          = "field14553";		//交易所手续费
var hysxfZH           = "field14554";		//会员手续费 zhhysxfZH
var jyyqsxfZH		  = "field14555";		//交易延期手续费
var crjZH			  = "field14556";		//出入金


var errorImgStr = '<img align="absMiddle" src="/images/BacoError.gif" complete="complete"/>';    //表单不能为空的图片
var pageObj = {   //页面对象   系统当前月，和客户记录信息
		thisMonth:"thisMonth",
		customer:{   									//客户信息模板
			name					: '',   							//客户姓名
			cardNumbe			: '', 		//身份证编号
		...
		getCustomerAndMonth : function(tradeNum){ //加载月份和客户数据 
			...
			jQuery.ajax({
				type: "post",
				url: "/yintech/wangQian/SXBRMEData.jsp",
				async : false,
		...
		pageInit : function(){
			var val = jQuery("#"+jyssxf).val(); //交易所手续费收取万分之
			computeObj.change();
			var numStr = NoToChinese(val);
			jQuery("#"+jyssxfZH).val(numStr);
			jQuery("#"+jyssxfZH +"span").html(numStr);
			
			var val2 = jQuery("#"+jyyqf).val();    //交易延期费收取万分之   
			var numStr = NoToChinese(val2);
			jQuery("#"+jyyqsxfZH).val(numStr);
			jQuery("#"+jyyqsxfZH +"span").html(numStr);

			var val3 = jQuery("#"+hysxf).val();    //会员手续费收取万分之
			var numStr3 = NoToChinese(val3);
			jQuery("#"+hysxfZH).val(numStr3);
			jQuery("#"+hysxfZH +"span").html(numStr3);

			var val4 = jQuery("#"+crjjz).val();    //出入金
			var numStr4 = NoToChinese(val4);
			jQuery("#"+crjZH).val(numStr4);
			jQuery("#"+crjZH +"span").html(numStr4);
		}
}

function checkIsO(){
		
	var scjyrq_v = jQuery("#"+tradeDate).val();//首次交易日期 .
...
jQuery(document).ready(function(){
	...
	//3.绑定字段校验条件
	/*
	var wfid = getUrlParam('workflowid');
	if(wfid==182||wfid==184){
		alert("请注意:此流程涉及的交易所手续费奖励,需要在相同交易账号有会员手续费减免至100%后方可提交,望知悉并确认,谢谢!");
	}
	...
	checkCustomize = function (){
		...
		var exchange_v = jQuery("#"+exchange).val();//交易所
	    var company_v = jQuery("#"+company).val();//所属公司

		if(workflowid==182||workflowid==184||workflowid==185||workflowid==186){
			if(exchange_v!='TPME'||company_v!='0'){
				alert("请检查当前交易账号的所属交易所和公司，并重新选择其他流程提交!");
				istj = false;
			}
		}
		
        if(workflowid==182||workflowid==184||workflowid==185){
    ...
    */
	...
	jQuery("#"+endDate).bindPropertyChange(function(){//优惠结束日期
		...
		if(( beginVal != undefined  &&  beginVal.length > 0) && ( endVal != undefined  &&   endVal.length > 0)){
			...
			if(isNewVal == 0){			//新客户限制	
			    ...
			    if(judgementMonth()){
					var endDateStr = getMonthBefore(endVal,3);
					var endDateObj   =  strParseDate(endDateStr);
					var beginDateObj =  strParseDate(beginVal);
	...
	jQuery("#"+jyssxf).change(function(){//交易所手续费奖励比例    (25,100]，最小变更单位25
		var val = jQuery("#"+jyssxf).val();
		var isPositiveNum = computeObj.isPositiveNum(  val / 25);
		if((val < 0 || val > 100)  ||  !isPositiveNum){
			jQuery("#"+jyssxf).val("");
			jQuery("#"+jyssxfZH).val("");
			jQuery("#"+jyssxfZH +"span").html("");
			alert("交易所手续费奖励比例    [25,100]，最小变更单位25");
		}else{
			computeObj.change();
	...
	var hysxf_v = jQuery("#"+hysxf).val();	

	if(hysxf_v>0||hysxf_v==0){

		computeObj.change();
		var numStr = NoToChinese(hysxf_v);
		jQuery("#"+hysxfZH).val(numStr);
		jQuery("#"+hysxfZH +"span").html(numStr);
    } 
	jQuery("#"+hysxf).change(function(){// 会员手续费优惠比例 
		var val = jQuery("#"+hysxf).val();
		
		if((val < 0 || val > 100)  ||  !computeObj.isPositiveNum(  val /5) ){
			jQuery("#"+hysxf).val("");

			jQuery("#"+hysxfZH).val("");
			jQuery("#"+hysxfZH+"span").val("");
			alert("会员手续费优惠比例  [5,100]，最小变更单位5");
		}else{
			computeObj.change();
			var numStr = NoToChinese(val);
			jQuery("#"+hysxfZH).val(numStr);
	...
	jQuery("#"+jyyqf).change(function(){//交易延期费优惠比例 
		var val = jQuery("#"+jyyqf).val();
		if((val <  0 || val > 100)  || !computeObj.isPositiveNum(  val / 25) ){
			jQuery("#"+jyyqf).val("");
			
			jQuery("#"+jyyqsxfZH).val("");
			jQuery("#"+jyyqsxfZH +"span").val("");
			alert("交易延期费优惠比例    [25,100]，最小变更单位25");
		}else{
			computeObj.change();
			var numStr = NoToChinese(val);
			jQuery("#"+jyyqsxfZH).val(numStr);
	...
	jQuery("#"+crjjz).change(function(){	//累计出入金净值不低于 正整数，默认为零
	...
	jQuery("#"+jyzh).change(function(){			//交易账号15位正整数
...
var computeObj = {
		...
		isSpecial3 : function(){   //判断是否总监特批    首次交易时间与当前日期的间隔,在三个月内 
			...
			var jyssxfVal 	  =  jQuery("#"+jyssxf).val();  	//交易所手续费奖励比例(%)
			var hysxfVal		 =  jQuery("#"+hysxf).val();  	//会员手续费优惠比例(%)
			var yqfVal	  =  jQuery("#"+jyyqf).val();   //延期费值
			//alert("crjjzVal : " + crjjzVal + ";    jyssxfVal:"+jyssxfVal +";     hysxfVal:"+hysxfVal + "; 	   yqfVal :"+yqfVal);
			var specialVal = false;
			var num = 0;
			if(yqfVal > 0 && yqfVal <= 100){
				specialVal = true;
				num = 1;
			}else if((jyssxfVal > 0 && jyssxfVal <= 100) && hysxfVal == 100){
				specialVal = true;
				num = 2;
			}else if(jyssxfVal == 0 && yqfVal == 0){
				if(crjjzVal.length>0){
					if(crjjzVal<30){
						specialVal = true;
						num = 3;
					}else if((crjjzVal >= 30 && crjjzVal < 50) && (hysxfVal > 40 && hysxfVal <= 100)){
						specialVal = true;
						num = 3;
					}else if((crjjzVal >= 50 && crjjzVal < 100) && (hysxfVal > 60 && hysxfVal <= 100)){
						specialVal = true;
						num = 4;
					}else if((crjjzVal >= 100 && crjjzVal < 200) && (hysxfVal > 80 && hysxfVal <= 100)){
						specialVal = true;
						num = 5;
					}else if(crjjzVal >= 200 && (hysxfVal > 0 && hysxfVal <= 100)){
						specialVal = false;
						num = 6;
					}
				}
		...
		isExceed : function(){   //判断是否超限
			...
			var exceedVal = false;
			if((jyssxfVal.length > 0 && hysxfVal.length > 0 ) &&  (jyssxfVal > 0 && jyssxfVal <= 100) && hysxfVal == 100){
				exceedVal = true;
		...
		isNewCustomer  :  function(){
			...
			var scjyDate  = strParseDate(tradeVal); //首次交易日期
			var beforeDate   = strParseDate(beforeDateStr);
