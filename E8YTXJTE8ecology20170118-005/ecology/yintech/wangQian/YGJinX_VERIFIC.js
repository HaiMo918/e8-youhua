var	jyzh			 = "field9230";	    //交易账号.
var dkyhzh 	         = "field9238";	    //打款银行账号 .

var jysxfsq1 	     = "field9245";	//交易手续费收取万分之.
var zhjysxfsq        = "field9244";	//综合会员收取手续费万分之.
//var jysxfsq          = "field9245";	//交易手续费总额  -----
var jyyqfysq         = "field9246";	//交易延期费收取万分之 .
var crjjz            = "field9249";	//累计出入金净值不低于 正整数，默认为空 .

var special		     = "field9253";     //是否特批 0-是,1-否 .
var exceed	         = "field9254";     //是否超限 0-是,1-否 .
var	scjyrq		     = "field9235";     //首次交易日期 .
var subDateId	     = "field9228";     //协议提交日期 .	

var name 			 =  "field9231";		//客户姓名
var cardNumbe		 =  "field9232";		//身份证编号
var customerId		 =	"field9233";		//客户ids
var exchange		 =  "field9234";		//所属交易所
var company			 =  "field9269";		//所属公司
var tradeDate		 =  "field9235";		//首次交易日期 .
var isNewCstm		 =  "field9236";		//是否新客户 .
var bank			 =	"field9237";		//开户绑定银行开户行.
var bindBankCard 	 =  "field9238";		//开户绑定银行账号/卡号.
var remittBank 		 =	"field9239";		//打款银行开户行.
var remittBankCard   =  "field9242";  	    //打款银行账号/卡号.
var isBindBankAlike  =  "field9243";		//打款银行是否与绑定银行账号/卡号相同 YGJX交易费用优惠协议Z 

var jyssxfZH         = "field9264";		//会员手续费
var zhhysxfZH        = "field9265";		//交易所手续费

//var hjsxfZH          = "field15059";	//合计手续费
var jyyqsxfZH	     = "field9266";		//交易延期手续费
var crjZH		     = "field9267";		//出入金

var beginDate 	     = "field9247";     //开始日期
var beginDateInput   = "field9262";		//开始日期中文表单
var beginDateSpan    = "field9262span";	//开始日期显示
var endDate 	     = "field9248"; 	//结束日期
var endDateInput     = "field9263";	    //结束日期中文表单
var endDateSpan      = "field9263span";	//结束日期显示

var nameZhVal 		 = "field9261";	    //姓名中文表单值
var nameZhSpan 	     = "field9261span";	//姓名中文显示
...
var pageObj = {   //页面对象   系统当前月份，和客户记录信息
		...
		getCustomerAndMonth : function(tradeNum){ //加载月份和客户数据 
			...
			jQuery.ajax({
				type: "post",
				url: "/yintech/wangQian/JinxGetData.jsp",
				async:false,
		...
		setCostomerToPage:function(){ //交易账号不为空就把返回的用户信息，填写到页面上
			...
			//jQuery("#" + cardNumbe+"span").(pageObj.customer.cardNumbe);
			...
			//jQuery("#" + remittBank+"span").html(pageObj.customer.remittBank);
            ...
            //jQuery("#" + remittBankCard+"span").html(pageObj.customer.remittBankCard);
...
jQuery(document).ready(function(){
	...
	jQuery("#"+jysxfsq1).change(function(){//会员手续费奖励比例    (0,100]，最小变更单位5
	...
	jQuery("#"+zhjysxfsq).change(function(){// 会员手续费优惠比例   (5,100]，最小变更单位5
		var val_zhjysxfsq = jQuery("#"+zhjysxfsq).val();
		
		if((val_zhjysxfsq < 0 || val_zhjysxfsq > 100)  ||  !computeObj.isPositiveNum(  val_zhjysxfsq / 5) ){
			...
			computeObj.change();
			var numStr = NoToChinese(val_zhjysxfsq);
			jQuery("#"+zhhysxfZH).val(numStr);
			jQuery("#"+zhhysxfZH +"span").html(numStr);
		}
	});

	jQuery("#"+jyyqfysq).change(function(){//交易延期费优惠比例  (0,100]，最小变更单位5
		var val_jyyqfysq = jQuery("#"+jyyqfysq).val();
		if((val_jyyqfysq <  0 || val_jyyqfysq > 100)  || !computeObj.isPositiveNum(  val_jyyqfysq / 5) ){
			jQuery("#"+jyyqfysq).val("");
			...
			computeObj.change();
			var numStr = NoToChinese(val_jyyqfysq);
			jQuery("#"+jyyqsxfZH).val(numStr);
			jQuery("#"+jyyqsxfZH +"span").html(numStr);
		}
	});
	...
	jQuery("#"+crjjz).change(function(){	//累计出入金净值不低于 正整数，默认为空
		var val_crjjz = jQuery("#"+crjjz).val();
		
		//var reg = /^[1-9]+[0-9]*]*$/;
		var reg =/^-?[0-9]\d*$/;
		if(!reg.test(val_crjjz)){
			jQuery("#"+crjjz).val("");
			jQuery("#"+crjZH).val("");
			...
			var crjjzChinese = NoToChinese(val_crjjz);
			jQuery("#"+crjZH).val(crjjzChinese);
			jQuery("#"+crjZH +"span").html(crjjzChinese);
			computeObj.change();
		}
	});
	
	jQuery("#"+jyzh).change(function(){
		//交易账号 							15位正整数
